...
function onClickOutside(e) {
    ...
    if (userMenuRef.value && !userMenuRef.value.contains(e.target))
        openUserDesktop.value = false;
}
onMounted(() => document.addEventListener("click", onClickOutside));
onBeforeUnmount(() => document.removeEventListener("click", onClickOutside));
watch(openMain, (o) => {
    if (!o) openGuestMobile.value = false;
});
...
<template>
    <nav
        class="sticky top-0 z-40 border-b border-white/20 bg-white/70 backdrop-blur-xl supports-[backdrop-filter]:bg-white/40"
    >
        <!-- Glow decorativo -->
        <div
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex h-16 items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    >
                        <ApplicationLogo
                            class="block h-8 w-auto fill-current text-[#0F3D3E] transition-transform group-hover:scale-105"
                        />
                        <span
                ...
                <div class="hidden items-center gap-1 md:flex">
                    ...
                    <div class="relative" ref="guestMenuRef">
                        ...
                        >
                            ...
                            <svg
                                ...
                                :class="openGuestDesktop ? 'rotate-180' : ''"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                            >
                                <path
                    ...
                    >
                        ...
                        >
                        <span
                            v-if="route().current('habitaciones.*')"
                            class="absolute inset-x-2 -bottom-1 h-0.5 rounded-full bg-gradient-to-r from-sky-400 to-indigo-400"
                        />
                    </Link>
                    ...
                    >
                        ...
                        >
                        <span
                            v-if="route().current('disponibilidad.*')"
                            class="absolute inset-x-2 -bottom-1 h-0.5 rounded-full bg-gradient-to-r from-sky-400 to-indigo-400"
                        />
                    </Link>
                ...
                <!-- Usuario -->
                <div
                    class="relative hidden items-center gap-3 md:flex"
                    ref="userMenuRef"
                >
                    <button
                    ...
                    >
                        <div
                            class="h-9 w-9 overflow-hidden rounded-full ring-1 ring-black/5"
                        >
                            <img
                            ...
                            <div
                                v-else
                                class="flex h-full w-full items-center justify-center bg-[#0F3D3E] text-xs font-semibold text-white"
                            >
                                {{
                        ...
                        <div
                            class="flex items-center gap-1 text-sm font-medium text-[#0F3D3E]"
                        >
                            <span class="max-w-[160px] truncate">{{
                                $page.props.auth.user.name
                            }}</span>
                            <svg
                                class="h-4 w-4 text-slate-500 transition group-hover:text-[#0F3D3E]"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                            >
                                <path
        ...
        <div
            v-show="openMain"
            class="md:hidden border-t border-black/5 bg-white/95 backdrop-blur"
        >
            <div class="space-y-1 py-2">
                ...
                <div>
                    ...
                    <div v-show="openGuestMobile" class="space-y-1 pb-2">
                        <Link
                            :href="routes.huesped"
                            class="block px-6 py-2 text-sm text-slate-700 hover:bg-slate-100"
                            >Huésped</Link
                        >
                        <Link
                            :href="routes.reservas"
                ...
                <div class="mt-2 border-t border-black/5 px-4 pt-2">
                    <div class="flex items-center gap-2">
                        <div
                            class="h-8 w-8 overflow-hidden rounded-full ring-1 ring-black/5"
                        >
                            <img
                                v-if="$page.props.auth.user.avatar_url"
                                :src="$page.props.auth.user.avatar_url"
                            ...
                            <div
                                v-else
                                class="flex h-full w-full items-center justify-center bg-[#0F3D3E] text-[10px] font-semibold text-white"
                            >
                                {{
                                    ($page.props.auth.user.name || "U")
                                        .split(" ")
                    ...
                    <div class="mt-2 space-y-1">
                        <Link
                            :href="routes.profileEdit"
                            class="block rounded-md px-2 py-2 text-sm text-slate-700 hover:bg-slate-100"
                            >Mi perfil</Link
                        >
                        <button
                            @click="chooseFile"
                            class="block w-full rounded-md px-2 py-2 text-left text-sm text-slate-700 hover:bg-slate-100"
                        >
                            Cambiar foto de perfil
                        </button>
                        <input
                        ...
                        <Link
                            ...
                            method="post"
                            as="button"
                            class="block w-full rounded-md px-2 py-2 text-left text-sm text-red-600 hover:bg-red-50"
                        >
                            Cerrar sesión
                        </Link>
                    </div>
