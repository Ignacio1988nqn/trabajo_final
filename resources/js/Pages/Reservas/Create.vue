                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="mt-2 opacity-90 text-lg">
                                    Complete los datos para registrar una nueva estadía
                                </p>
                            </div>
                                ...
                                <div class="relative">
                                    <div class="flex gap-3">
                                        <div class="flex-1 relative">
                                            <input type="text" v-model="buscador.state.busq" @input="buscador.onInput"
                                                @keydown="buscador.onKeydown" @blur="buscador.blurConDelay"
                                                placeholder="Buscar por Apellido, Nombre, DNI, Razón Social o CUIT..."
                                                class="w-full px-5 py-4 pr-12 rounded-xl border-2 border-gray-300 focus:border-indigo-500 
                   focus:ring-4 focus:ring-indigo-100 transition text-lg" autocomplete="off" />
                                        ...
                                        <button type="button" @click="buscador.limpiar"
                                    ...
                                    <div v-if="buscador.state.abierto && buscador.state.opciones.length"
                                        class="absolute z-50 mt-2 w-full bg-white rounded-xl shadow-2xl border border-gray-200 max-h-80 overflow-y-auto">
                                        <button v-for="(op, i) in buscador.state.opciones" :key="op.id" type="button"
                                            ...
                                            :class="{ 'bg-indigo-100': i === buscador.state.highlighted }">
                                            <div>
                                                <p class="font-semibold text-gray-800">{{ op.display }}</p>
                                                <p class="text-sm text-gray-500">{{ op.tipo === 'persona' ? 'DNI' :
                                                    'CUIT' }}:
                                                    {{ op.doc }}</p>
                                            </div>
                                            <span class="text-xs px-3 py-1 rounded-full"
                                                :class="op.tipo === 'persona' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'">
                                                {{ op.tipo }}
                                            </span>
                                        </button>
                                    </div>
                                ...
                                <p v-if="form.errors.huesped_id" class="mt-3 text-red-600 font-medium">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="text-xl font-bold text-gray-800">Habitaciones asignadas</h3>
                                    <button type="button" @click="addAsignacion"
                                ...
                                <div v-for="(a, idx) in form.asignaciones" :key="idx" class="mb-8">
                                    <div
                                        ...
                                        <div class="flex items-center justify-between mb-5">
                                            ...
                                            <button type="button" @click="removeAsignacion(idx)"
                                        ...
                                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                            ...
                                            <div>
                                                ...
                                                <input type="date" v-model="a.fecha_inicio" :min="hoy"
                                            ...
                                            <div>
                                                ...
                                                <input type="date" v-model="a.fecha_fin" :min="a.fecha_inicio || hoy"
                                                    class="w-full px-4 py-3 rounded-lg border-2 border-gray-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition"
                                                    required />
                                            </div>
                                            ...
                                            <!-- Habitación -->
                                            <div>
                                                <label class="block text-sm font-semibold text-gray-700 mb-2">Habitación
                                                    disponible</label>
                                                <select v-model="a.habitacion_id"
                                                    class="w-full px-4 py-3 rounded-lg border-2" :class="[
                                                        a.habitacion_id && isSeleccionDisponible(idx, a.habitacion_id)
                                                            ? 'border-green-500 bg-green-50'
                                                            : a.habitacion_id ? 'border-red-500 bg-red-50' : 'border-gray-300'
                                                    ]" :disabled="!a.fecha_inicio || !a.fecha_fin">
                                                    <option value="">Seleccione fechas primero...</option>
                                                    <option v-for="hab in disponibles[idx] || []" :key="hab.id"
                                                        :value="hab.id">
                                                        {{ hab.numero }} — {{ hab.tipo }} ({{ hab.capacidad }} pers.)
                                                    </option>
                                                </select>

                                                <div v-if="a.habitacion_id" class="mt-2 flex items-center gap-2">
                                                    <span class="px-3 py-1 rounded-full text-xs font-bold" :class="isSeleccionDisponible(idx, a.habitacion_id)
                                                        ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                                                        {{ isSeleccionDisponible(idx, a.habitacion_id) ? 'Disponible' :'Ocupada en esas fechas' }}
                                                    </span>
                                                </div>
                                                <p v-if="form.errors[`asignaciones.${idx}.habitacion_id`]"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <label class="block text-lg font-semibold text-gray-800 mb-3">Estado de la
                                        reserva</label>
                                ...
                                <div>
                                    <label class="block text-lg font-semibold text-gray-800 mb-3">Observaciones
                                        (opcional)</label>
                                    <textarea v-model="form.observaciones" rows="4"
                                        placeholder="Ej: Llegada tarde, pedido de cuna, allergy..."
                                        class="w-full px-5 py-4 rounded-xl border-2 border-gray-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition"></textarea>
                                </div>
                            </div>

                            <!-- Botones finales -->
                            <div class="flex justify-end gap-5 pt-8 border-t-2 border-gray-200">
                                ...
                                <button type="submit" :disabled="form.processing"
                                    ...
                                    <svg v-if="form.processing" class="animate-spin h-6 w-6" fill="none"
                                        viewBox="0 0 24 24">
                                    ...
                                    <span>{{ form.processing ? 'Guardando reserva...' : 'Crear Reserva' }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
